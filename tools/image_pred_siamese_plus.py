...
from Models import Siam_NestedUNet_Conc
...
class predImage(nn.Module):
    def __init__(self, num_layer,input_size):
        ...
        self.layers.append(nn.Sequential(
                        nn.Conv2d(6, 64, kernel_size=3, padding=1, stride=1,bias=True),
        ...
        self.head_conv = nn.Sequential(
            nn.Conv2d(64, input_size, kernel_size=input_size//8, bias=True),
...
if __name__ == '__main__':
    ...
    model = Siam_NestedUNet_Conc(3,3)#predImage(0,input_size)
    model.train()
    model.cuda()
    criteria = torch.nn.L1Loss().cuda()#nn.MSELoss(reduction='mean')
