...
def plot_curve(log_dicts, args):
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            if metric in ['mIoU', 'mAcc', 'aAcc']:
                ...
                if 'loss' in args.out:
                    best = min(plot_values)
                else:
                    best = max(plot_values)
                print('Best {} is at iter {}:{}'.format(metric, plot_iters[plot_values.index(best)],best))
        plt.legend()
        plt.grid()
    ...
    if args.out is None:
        ...
        out = args.json_logs[0].replace(args.json_logs[0].split('/')[-1], args.out)
        plt.savefig(out)
        plt.cla()
