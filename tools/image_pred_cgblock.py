...
class predImage(nn.Module):
    def __init__(self, num_layer, input_size):
        ...
        self.head_conv1 = nn.Sequential(
            ...
            nn.PReLU(512),
            nn.Conv2d(512, 64*64*3, kernel_size=64, bias=True,groups=256),
            #nn.PReLU(input_size),
        )
    ...
    def forward(self, ok,ref):
        H,W = ok.shape[1:3]
        out = self.model(ok, ref)
        out = self.head_conv1(torch.cat([out[0][-1],out[1][-1]], dim=1))
        #out = self.head_conv2(out.view(4,1,64,64))
        out = F.interpolate(out.view(4,3,64,64), size=ok.shape[2:], mode='bilinear')
        # out = [F.interpolate(out[i],size=ok.shape[2:], mode='bilinear')
        #        for i in range(len(out))]
        # out = torch.cat(out, dim=1)

        return out#out.view(out.shape[0], 3, self.size, self.size)
