...
from tools.fmap_visualize import FeatureMapVis_siamese,FeatureMapVis
from pathlib import Path
from tools.fmap_visualize import show_tensor, show_img
import numpy as np
import os
import cv2
import torch
...
def create_featuremap_vis(model=None, use_gpu=True, init_shape=(512, 512, 3),data=None):
    #model.forward = partial(forward, model)
    featurevis = FeatureMapVis_siamese(model, use_gpu, data)
...
def _show_save_data_siamese(featurevis, feature_indexs, data, fmaps = None):
    show_datas = []
    raw_features = []
    imgs = data['img'][0].detach().cpu().numpy().transpose([0, 2, 3, 1])
    if fmaps is None:
        for feature_index in feature_indexs:
            feature_maps = featurevis.run(data, feature_index=feature_index)[0]
            if feature_maps.shape[0]==1:
                feature_maps = [feature_maps.squeeze(0)]
            else:
                feature_maps = [feature_maps[:1,::][0],feature_maps[1:,::][0]]
            for fea in feature_maps:
                assert fea.dim() == 3, 'Feature dim from index-%d is not 3, please check!' % feature_index
                show_data = show_tensor(fea, resize_hw=data['img'][0].shape[2:], show_split=False, is_show=False)[0]
                show_datas.append(show_data)
                raw_features += [_.cpu().detach().numpy() for _ in feature_maps]
    else:
        for i in range(len(fmaps)):
            ...
            for img, feature_map in zip(imgs, fmaps[i]):
                ...
                am_data.append(data)
            show_datas.append(am_data)
    return show_datas, raw_features
