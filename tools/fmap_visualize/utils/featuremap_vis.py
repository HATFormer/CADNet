...
class ModelOutputs(object):
    ...
    def __call__(self, x, index=[-1], vis=False, save_gradient_flag=True):
        ...
        output = self._net(return_loss=False, **x)
        # 移除hook
...
class BaseActivationMapping(object):
    ...
    def _print_model_structure(self, print_summary=True):
        ...
        if self._use_gpu:
            extra = extra.cuda()
        with torch.no_grad():
            self._net(return_loss=False, **self.data)
        # 删除hook，防止影响
        ...
        if print_summary:
            tmp = self.data.copy()
            tmp.__delitem__('img')
            tmp['return_loss'] = False
            summaryX.summary(self._net, self.data['img'], **tmp)
...
class FeatureMapVis_siamese(BaseActivationMapping):
    def __init__(self, net, use_gpu=True, data=None):
