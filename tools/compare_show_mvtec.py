...
if __name__ == '__main__':
    seg_root = '/home/hc/lby/ccd/mmsegmentation/work_dirs/segformer_mvtec_all/show48711'
    test_list = open('../../../data/mvtec_ad/test.txt','r').readlines()

    name_list = [extract_list(os.path.join(root, 'mixed_output')) for root in roots]
    img_list,Errs = extract_img(name_list)
    show(img_list, Errs,
         names=['OK', 'NG', 'GT', 'distmap w/o nocontra', 'Ours', 'Ours', 'DANet', 'Deeplab', 'PSPNet', 'FCN'])
    print('Complete!')
    for _ in Errs:
        print(_)