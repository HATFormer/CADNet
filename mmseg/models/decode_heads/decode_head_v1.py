...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.margin1 = margin1 = 0.3
        self.margin2 = margin2 = 2.2
        self.eps = 1e-6
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        if type(seg_logits) == tuple and type(seg_logits[1]) != list:
            losses = self.losses_withCLS(seg_logits[0], seg_logits[1], gt_semantic_seg)
        elif type(seg_logits) == tuple and type(seg_logits[1]) == list:
    ...
    @force_fp32(apply_to=('x1', 'x2'))
    def contra_losses(self, seg_logit, features, seg_label, seg_weight=None):
        """Compute segmentation loss."""
        loss = dict()
        tmp = []
        for _ in features+[seg_logit]:
            tmp.append(resize(
                input=_,
                size=seg_label.shape[2:],
                mode='bilinear',
        ...
        loss_pos = (1 - seg_label) * (mdist_pos.pow(2))
        loss_neg = seg_label * (mdist_neg.pow(2))
        # TODO: better way for device assignment
        loss_contra = torch.tensor(torch.mean(loss_pos + loss_neg)).cuda()
