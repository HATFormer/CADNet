    ...
    def forward(self, out, distance):
        ...
        if self.cam:
            if self.thatt:
                ...
                """ line_v8"""
                #out = torch.cat([distance, out], dim=1)
                #distance = self.normMask1(distance)
                # hw_ca = self.hw_ca(intra * distance)
                #out = self.d_ca(out * distance) * (out + hw_ca.repeat(1, 4, 1, 1))
                #out = distance * self.d_ca(out) * out
                """ abpt_v3"""
                # out = torch.cat([distance,out],dim=1)
                # hw_ca = self.hw_ca(out)
                # out = self.d_ca(out) * (out + hw_ca)
                """ abpt_v4"""
                distance = self.normMask1(distance)
                out = distance * self.d_ca(out) * out
                "2023.1.1 for ablation4"
                # hw_ca_1 = self.hw_ca_1(intra+distance)
                # out = self.d_ca(out + distance) * (out + hw_ca_1.repeat(1, 4, 1, 1))
            else:
                ### line_v3
