...
class cam_head(nn.Module):
    def __init__(self, mid_ch=None, out_ch=None, cam=True, thatt=False, cross=False, HW=512):
        ...
        if self.cam:
            if not self.cross:
                self.hw_ca = CoordAtt(mid_ch, mid_ch, h=HW, w=HW, groups=16)
                self.d_ca = ChannelAttention(mid_ch, ratio=16)
                self.hw_ca_1 = CoordAtt(mid_ch // 4, mid_ch // 4, h=HW, w=HW, groups=16 // 4)
        ...
        if self.cross:
            ...
            self.conv_final = nn.Conv2d(mid_ch, 1, kernel_size=1)
        else:
    ...
    def forward(self, out, distance):
        ...
        if self.cam:
            ...
            if self.thatt:
                ...
                out = distance * self.d_ca(out) * out
...
class AttFCNHead(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        self.att_head = cam_head(mid_ch=self.channels, out_ch=self.num_classes,
                                 cam=cam, thatt=thatt, cross=cross, HW=HW)
    ...
    def forward(self, inputs):
        ...
        if type(self.in_index) != list:
            #csize = x.shape[1]//4
            #x = x[:,self.in_index*csize:(self.in_index+1)*csize,:,:]
