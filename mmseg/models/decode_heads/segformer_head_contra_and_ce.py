...
class SegformerHeadContra_and_CE(BaseDecodeHead):
    ...
    def __init__(self, interpolate_mode='bilinear', change_att=False,
        ...
        self.fusion_conv = ConvModule(
            ...
            kernel_size=1,
            norm_cfg=self.norm_cfg)
        self.cls_contra = nn.Conv2d(self.channels, 1, kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        if self.change_att:
            out = self.cad_head(outs, distance)

            out = self.fusion_conv(out)
        else:
            out = self.fusion_conv(torch.cat(outs,dim=1))

        return self.cls_seg(out), [distance, self.cls_contra(out)], 'contra_and_ce'
