...
class cam_head(nn.Module):
    def __init__(self, mid_ch=None, out_ch=None, cam=True, thatt=False, cross=False):
        ...
        if self.cross:
            ...
            self.conv_final = nn.Conv2d(mid_ch, 2, kernel_size=1)
        else:
    ...
    def forward(self, out, distance):
        ...
        if self.cam:
            if self.thatt:
                ...
                hw_ca_1 = self.hw_ca_1(intra)
                out = self.d_ca(out) * (out + hw_ca_1.repeat(1, 4, 1, 1))
...
class AttFCNHead_HWAtt(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        self.cam = cam
        self.thatt=thatt
        super(AttFCNHead_HWAtt, self).__init__(**kwargs)
        if num_convs == 0:
