...
class EfficientMultiheadAttention(MultiheadAttention):
    ...
    def __init__(self,
        ...
        self.attn.batch_first = False#True
    def forward(self, x, hw_shape, identity=None):
...
class SiameseMixVisionTransformer(BaseModule):
    ...
    def forward(self, x):
        bsize = x.shape[0]//2
        tmps = self.sub_forward(x)
        outs = [[],[]]


        for tmp in tmps:
            outs[0].append(tmp[:bsize,::])
            outs[1].append(tmp[bsize:,::])
        mid1,mid2 = self._transform_inputs(outs[0]),self._transform_inputs(outs[1])
        n, c, h, w = mid1.shape
        distance = self.various_distance([mid1,mid2]).view(n, 1, h, w)
        outs = [x-y for x,y in zip(outs[0],outs[1])]
        return outs, distance