...
def cgblock(in_ch, out_ch, dilation=2, reduction=8, conv2d=False, downsample=False):
...
class CADNet_small(nn.Module):
        ...
        if not self.diffFPN:
            ...
            for i in range(1, pairs - 1):
                self.top_down_convs.append(
                    cgblock(filters[i + 1], filters[i], dilation=dilations[i + 1], reduction=reductions[i + 1],
                            conv2d=conv2d))
    ...
    def various_distance_sigmoid(self, feature_pairs):
        fea1, fea2 = feature_pairs
        n, c, h, w = fea1.shape
        fea1_rz = torch.transpose(fea1.view(n, c, h * w), 2, 1)
        fea2_rz = torch.transpose(fea2.view(n, c, h * w), 2, 1)
        dist = F.pairwise_distance(fea1_rz, fea2_rz, p=2).view(n, 1, h, w)
        return self.classifier_loc(dist * (fea1 - fea2))
    ...
    def forward(self, x):
        ...
        output = [x1_0A, x1_0B, x2_0A, x2_0B, x3_0A, x3_0B, x4_0A, x4_0B]

        mid = self._transform_inputs((x1_0, x2_0, x3_0, x4_0))
        n, c, h, w = mid.shape
        n = n // 2
        mid = [mid[:n, ::], mid[n:, ::]]
        ...
        x = self._transform_inputs((x0_0, x0_1, x0_2, x0_3))
