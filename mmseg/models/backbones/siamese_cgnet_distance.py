...
class SiameseCGNet(nn.Module):
    ...
    def forward(self, x):
        ...
        distance = self.get_distance((x0_0, x1_0, x2_0, x3_0, x4_0))
        ...
        if not self.diffFPN:
            ...
            pairs = len(output)//2
            tmp = [self.lateral_convs[i](torch.cat([output[i * 2], output[i * 2 + 1],
                    F.interpolate(distance, size=output[i * 2].shape[2:],
                              align_corners=False, mode='bilinear')], dim=1))
                   for i in range(pairs)]
        ...
        return (x0_1, x0_2, x0_3, x0_4), distance
